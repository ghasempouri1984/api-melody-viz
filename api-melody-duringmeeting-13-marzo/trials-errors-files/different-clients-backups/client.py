...
def plotChart(query, chart_type, endpoint, format):
    ...
    if "count" in variable_names:
        y_var = "count"
        x_var = [var for var in variable_names if var != "count"][0]
    ...
    if chart_type == 'bar':
        ...
        fig = px.bar(x=labels, y=counts, labels={x_var: '', y_var: ''}, title="Number of items", height=500, width=500, category_orders={x_var: sorted(labels)})
