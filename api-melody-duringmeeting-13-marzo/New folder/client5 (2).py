...
#def plotChart(query: str, chart_type: str, endpoint: str, format: str, scatter_x: str = None, scatter_y: str = None):
def plotChart(query: str, chart_type: str, scatter_x: str, scatter_y: str, endpoint: str, format: str):
    ...
    x_data, y_data, artwork_labels = _prepare_data(x_var, y_var, values, data)
    ...
    # If there's only one count, return count as text
        ...
        df = pd.DataFrame({x_var: x_data, y_var: y_data, 'artwork_labels': artwork_labels})
...
def _prepare_data_0(x_var: str, y_var: str, values: dict, data: list) -> tuple:
...
def _prepare_data(x_var: str, y_var: str, values: dict, data: list) -> tuple:
    #x_data = [val if val else None for val in values.get(x_var, [])]
    #y_data = [val if val else None for val in values.get(y_var, [])]
    x_data = [float(val) if val else None for val in values.get(x_var, [])]
    y_data = [float(val) if val else None for val in values.get(y_var, [])]
    artwork_labels = [get_artwork_label(artwork_uri, data) for artwork_uri in values.get('artwork', [])]

    return x_data, y_data, artwork_labels


def _create_visualization_0(df: pd.DataFrame, x_var: str, y_var: str, chart_type: str, format: str):
    if chart_type == 'bar':
        ...
        #fig = px.scatter(df, x=x_var, y=y_var, hover_data=['artwork_labels'], labels={x_var: x_var, y_var: y_var}, title=f"{x_var} vs {y_var}", height=500, width=500)
