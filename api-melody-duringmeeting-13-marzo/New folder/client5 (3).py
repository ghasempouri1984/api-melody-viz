...
def _prepare_data_(x_var: str, y_var: str, values: dict, data: list) -> tuple:
...
def _prepare_data(x_var: str, y_var: str, values: dict, data: list) -> tuple:
    x_data = [convert_to_number(val) for val in values.get(x_var, [])]
    y_data = [convert_to_number(val) for val in values.get(y_var, [])]
    artwork_labels = [get_artwork_label(artwork_uri, data) for artwork_uri in values.get('artwork', [])]
