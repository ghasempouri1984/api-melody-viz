...
def check_variables_existence(df: pd.DataFrame, required_vars: List[str]) -> bool:
...
def plotChart(query: str, chart_type: str, scatter_x: str, scatter_y: str, scatter_label: str, endpoint: str, format: str):
    ...
    if chart_type == 'scatter':
        ...
        print(df)
        Handle missing values for Scatterplot
        df = handle_missing_values(df)
        print("Debug Step 4 - DataFrame after handle_missing_values:")
        print(df)
        # Check for essential variables for Scatterplot
        required_vars = [x_var, y_var]
        if not check_variables_existence(df, required_vars):
            print("Essential variables are missing.")
            return "Essential variables are missing.", None
    else:
